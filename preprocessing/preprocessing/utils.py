...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                value_counts = df['tag_name'].value_counts()
                valid_tags = value_counts[value_counts >= x].index
    ...
    def get_train_batch(self):
        ...
        next_x = self.preprocess_text(self.x_train[i * self.batch_size:(i + 1) * self.batch_size].values)
        # TODO: CODE HERE
    ...
    def get_test_batch(self):
        ...
        next_x = self.preprocess_text(self.x_test[i * self.batch_size:(i + 1) * self.batch_size].values)
